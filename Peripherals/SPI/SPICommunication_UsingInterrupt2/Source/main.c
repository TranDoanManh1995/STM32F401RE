...
#define LED_GREEN2									GPIO_Pin_11
#define LED_GREEN2_PIN								11
...
static void SPI_ReadData(SPI_TypeDef *SPIx, uint16_t *data);
...
/* Global variable -------------------------------------------------*/
uint16_t Receive_Data_Slave = 0;
uint16_t Receive_Data_Master = 0;
...
int main()
{
	...
	while(1)
	{
		...
		{
			SPI_TransmitData(SPI_MASTER, SLAVE_DATA_CHECK);
		}

		if(Receive_Data_Slave == SLAVE_DATA_CHECK)
		{
			for(int i = 0; i <5; i++)
			{
				LedControl_SetStatus(LED_PORT, LED_GREEN1_PIN, GPIO_PIN_SET);
				Delay(1000);
				LedControl_SetStatus(LED_PORT, LED_GREEN1_PIN, GPIO_PIN_RESET);
				Delay(1000);
			}

			Receive_Data_Slave = 0;

			SPI_TransmitData(SPI_SLAVE, MASTER_DATA_CHECK);

			SPI_ReadData(SPI_MASTER, &Receive_Data_Master);

			if(Receive_Data_Master == MASTER_DATA_CHECK)
			{
				LedControl_SetStatus(LED_PORT, LED_GREEN2_PIN, GPIO_PIN_SET);
				Delay(1000);
				LedControl_SetStatus(LED_PORT, LED_GREEN2_PIN, GPIO_PIN_RESET);
				Delay(1000);
...
void SPI1_IRQHandler(void)
{
	...
	{
		 Receive_Data_Slave = SPI_I2S_ReceiveData(SPI_SLAVE);
	}
...
static void Led_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = LED_GREEN1 | LED_GREEN2;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;
